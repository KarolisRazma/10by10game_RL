...
from src.agents.custom_improved_agent import CustomImprovedAgent
...
class GameInterface:
    ...
    def __init__(self):
        ...
        self.custom_improved_agent = CustomImprovedAgent(name=CustomImprovedAgentParameters.name,
                                                         graph=self.graph_improved_agent_1,
                                                         learning_algorithm=RLearning(
                                                             CustomImprovedAgentParameters.discount_rate,
                                                             CustomImprovedAgentParameters.learning_rate),
                                                         exploit_growth=CustomImprovedAgentParameters.exploit_growth,
                                                         explore_minimum=CustomImprovedAgentParameters.explore_minimum,
                                                         is_improved_exploitation_on=
                                                         CustomImprovedAgentParameters.is_improved_exploitation_on,
                                                         exploit_to_closed_state_rate=
                                                         CustomImprovedAgentParameters.exploit_to_closed_state_rate,
                                                         )
    ...
    def process_set_agents_option(self):
        while True:
            ...
            #     self.environment.set_agent_2(self.random_walker_agent_1)
                ...
                self.environment.set_agent_1(self.custom_improved_agent)
                self.environment.set_agent_2(self.random_walker_agent_1)
            elif choice == 3:
    ...
    def process_start_episode_option(self):
        while True:
            ...
            for i in range(episodes):
                ...
                if isinstance(self.environment.agents[0], ImprovedAgent) or \
                        isinstance(self.environment.agents[0], CustomImprovedAgent):
                    agent = self.environment.agents[0]
                    agent.eval_path_after_episode(dynamic_state_closure_depth)

                # Evaluate ImprovedAgent path
                if isinstance(self.environment.agents[1], ImprovedAgent) or \
                        isinstance(self.environment.agents[1], CustomImprovedAgent):
                    agent = self.environment.agents[1]
                    agent.eval_path_after_episode(dynamic_state_closure_depth)

                # self.game_state_closure_handler.set_target_agent(self.improved_agent_1)
                # self.game_state_closure_handler.set_target_agent(self.custom_improved_agent)
